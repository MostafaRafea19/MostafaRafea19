### Hi there 👋 I'm Mostafa Rafea
**Senior Backend Engineer** specialized in High-Availability Distributed Systems.
*Passionate about AI Engineering & Data Science.*

I build scalable architectures using **Laravel**, **MongoDB**, **Redis**, and **RabbitMQ**, while expanding into **Python** for AI and Machine Learning.

🔭 **Current Focus:**
* 🏗️ **Architecture:** Designing Event-Driven Microservices (`EventCore`).
* 🦸 **Automation:** Creating AI Content Pipelines for media (`Comic-AI-Factory`).

🛠️ **Tech Stack:**
* **Backend:** PHP 8.3, Laravel 11, Node.js
* **AI/Data:** Python, FastAPI, Pandas, Scikit-Learn
* **Infrastructure:** Docker, RabbitMQ, Redis, MongoDB
